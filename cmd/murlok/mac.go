...
import (
	...
	"github.com/maxence-charriere/murlok/internal/file"
...
func clean(ctx context.Context, args []string) {
	c := cleanConfig{}
...
func (pkg *MacPackage) init() (err error) {
	...
	pkg.webDir = filepath.Join(pkg.Sources, "web")
...
func (pkg *MacPackage) readSettings(ctx context.Context) error {
	...
	os.Setenv("MURLOK_BUILD", settingsPath)
	defer os.Unsetenv("MURLOK_BUILD")
